...
template <class Key, class Data> int
EfficientMap <Key, Data> :: Remove (Key &key, Key &removedKey, Data &removedData,OrderMaker &order) {
	MoveToStart ();
	OrderMaker o = order;
...
//	temp->data.Swap (data);
	...
	for (int i = list->curDepth - 1; i >= 0; i--) {
		...
		while (1) {
			...
			// or we find a larger item
			/*sif (!CurrentKey(i).LessThan (temp->key))
				break;*/
