...
private:
	...
	EfficientMap<Record,int> rightMap;
	EfficientMap<Record,int> leftMap;
	int data = 0;
	int data2 = 0;
	int counter = 0;
	int equals = 0;
	//cout<<"Record size: "<<_record.GetSize()<<endl;//size of record
	Record recordL;// record for the left
	Record recordR;// record for the rights
	Record recL;
	Record recR;
	Record fin;
	Record fin2;
...
private:
	...
	TwoWayList<Record> rlist;
	bool first = true;
	int start = 0;
	Record r;
	//EfficientMap<Record,int>emap;

public:
	DuplicateRemoval(Schema& _schema, RelationalOp* _producer);
	//EfficientMap<int,string>emap;
...
private:
	...
	EfficientMap<Record,double> recmap;
...
public:
	...
	void ExecuteQuery() {Record r; while(root->GetNext(r));}
